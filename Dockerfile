...
RUN poetry config virtualenvs.create false && poetry install --no-dev --no-root --no-interaction --no-ansi && poetry export -f requirements.txt --output requirements.txt --without-hashes && pip wheel --no-cache-dir --no-deps --wheel-dir /tmp/wheels -r requirements.txt
